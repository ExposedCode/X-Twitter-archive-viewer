...
window.addEventListener('resize', function() {
  winMode = Math.min(window.innerWidth, window.innerHeight);
  if(winMode > 768)
  {
    unloadFile("scripts/css/profile.mobile.css");
    loadScriptMarkup("scripts/css/profile.desktop.css");
    //viewport.setAttribute('content', 'width='+window.innerWidth);
  }
  else
  {
    unloadFile("scripts/css/profile.desktop.css");
    loadScriptMarkup("scripts/css/profile.mobile.css")
...
function startLoader()
{
  ...
  {
    if(document.documentElement.clientWidth < 768) loadScriptMarkup("scripts/css/profile.mobile.css");
    else if(document.documentElement.clientWidth > 768)
      loadScriptMarkup("scripts/css/profile.desktop.css")
    
    $(document.body).append("<div class=\"app-loader\" style=\"height:105vmax;width:100%;position:fixed;top:0px;left:0px;background-color:#fff;z-index:5;overflow-x:none;overflow-y:none\"><center style=\"position:relative;margin:0;top:50%;transform:translateY(-50%);-ms-transform:translateY(-50%)\"><i class=\"fa fa-twitter\" style=\"font-size:70px;color:#1DA1F2\"></i></center></div><div class=\"image-viewer\"><img src=\"\"></div>");
     dir = window.opt.data_folder.split("/");
     
     loadScriptMarkup(dir[0]+"/imgalt.js");
     loadScriptMarkup(dir[0]+"/retweet.js");
  }
}

function FileExists(url, variable=null, callback=null, loop=0)
{
  ...
  else if(vdoType.indexOf(fileType[fileType.length-1])>-1)
  {
    ...
    vdoFile.addEventListener("loadeddata", () => {
      if(document.body.removeChild(vdoFile))
      {
         callback(true, variable, url, loop);
      }
    });
  }
}
  
function loadScriptMarkup(FILE_URL, feedback=null, async = true)
{
  ...
  {
    if(filename.indexOf("mobile")>-1)
    {
      $("link[id^='ec0d1bc72b6f76e97f5afcc75b22559e']").prop("disabled", false);
      $("link[id^='2b2f493564cb3a1eb6640e63201c5bf9']").prop("disabled", true);
    }
    else if(filename.indexOf("desktop")>-1)
    {
      $("link[id^='2b2f493564cb3a1eb6640e63201c5bf9']").prop("disabled", false);
      $("link[id^='ec0d1bc72b6f76e97f5afcc75b22559e']").prop("disabled", true);
    }
  }
}

function unloadFile(FILE_URL)
{
  filename = FILE_URL.substring(FILE_URL.lastIndexOf("\/")+1, FILE_URL.length);
  filetype = filename.substring(filename.lastIndexOf(".")+1, filename.length);
  if(filetype.toLowerCase()=="js")
  ...
  {
    $("link[id^='"+CryptoJS.MD5(FILE_URL).toString()+"']").remove();
  }
}
          ...
          click : function() {
            ...
            if($(this).find('input[id="data-folder"]').val().length>3)
            {
              if($(this).find('input[id="data-folder"]').val().substring(0,1)=="/")
              {
                ...
                loadScriptMarkup(folder[1]+"/profile.js", function(stat) {
                  if(!stat)
                  {
                  ...
                  else
                  {
                   ...
                   loadScriptMarkup(folder[1]+"/manifest.js", function(stat) {
                      if(!stat)
                      {
              ...
              {
                folder = $(this).find('input[id="data-folder"]').val().split("/");
                loadScriptMarkup(folder[0]+"/profile.js", function(stat) {
                  if(!stat)
                  {
                    alert("Folder not found !!!");
                    $(this).find('input[id="data-folder"]').focus();
                  ...
                  else
                  {
                    ...
                    loadScriptMarkup(folder[0]+"/manifest.js", function(stat) {
                      if(!stat)
                      {
...
var addCssRules = (function(id){
    var style = document.createElement('style'), styleTxt = [];
    style.type = 'text/css';
    style.id = CryptoJS.MD5(id).toString();
...
function isMobile()
{
  ...
  first_req = regex.test(navigator.userAgent);
  
  second_req = 'ontouchstart' in window || navigator.maxTouchPoints > 0;
  
  third_req = Math.min(window.screen.width, window.screen.height) < 768;
}
...
function isContains(str, arr)
{
  ...
  if(arr!=null && arr.length>0)
  {
    ...
    {
      if(arr[i].indexOf(str)>=0) result = true;
    }
  }
  return result;
}
...
function updateLinks(d)
{
  ...
  else if(typeof(d.tweet)!='undefined')
  {
    ...
    if(dump.length>0)
    {
      ...
      {
        let urls = new RegExp(/(http|https|ftp):\/\/(\S*)/ig);
        lnk = d.tweet.full_text.match(urls);
        exc = d.tweet.full_text.match(new RegExp(/(•|●|■|◆|√|✓|°|\+|\*|\-)\s(.*)+/gm));
        d.tweet.entities.urls.forEach(m => {
          if(m.url==lnk[lnk.length-1] && window.navigator.onLine && !isContains(m.url, exc) && d.tweet.full_text.indexOf(m.url)>(d.tweet.full_text.length-50))
          {
            if(m.expanded_url.match(new RegExp(/https?:\/\/((www|m)\.)?(youtube\.com|youtu\.be)\/(.*)+/gm)) && window.navigator.onLine)
            {
              ytid = m.expanded_url.slice(m.expanded_url.lastIndexOf("\/")+1, m.expanded_url.length);
              if(ytid.indexOf("watch")>=0)
              {
                ytid = ytid.slice(ytid.lastIndexOf("v=")+2, ytid.length);
              }
              dump = dump.replace(m.url, "<div id=\"ytprev\"><iframe src=\"https://www.youtube.com/embed/"+ytid+"\" allow=\"accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share\" allowfullscreen></iframe></div>");
            }
            else if(m.expanded_url.match(new RegExp(/^http[s]:\/\/[a-zA-Z]+\.wikipedia\.org\/(.*)+/g)))
            {
              m.expanded_url;
            }
            //else alert(m.expanded_url)
          }
          else
        ...
        //if(last.match(new RegExp(/^http[s]:\/\/[youtube.com|youtu.be]\/*/g))) alert(last)
...
function updateMedia(txt, d)
{
  if(txt.length>0)
  {
    if(typeof(d.tweet.extended_entities)!=='undefined')
    {
      ...
      var desc = [];
      //window.opt.cache = [];
      video_len = d.tweet.extended_entities.media.filter(a=>a.type=="video" || a.type=="animated_gif").length;
      photo_len = d.tweet.extended_entities.media.filter(a=>a.type=="photo").length;
      for(var z=0; z<d.tweet.extended_entities.media.length; z++)
      {
        if(d.tweet.extended_entities.media[z].type=="video" || d.tweet.extended_entities.media[z].type=="animated_gif")
        {
          ...
          for(var s=0; s<d.tweet.extended_entities.media[z].video_info.variants.length; s++)
          {
           ...
           var vdoFname = (fName.length>0)?window.opt.data_folder+d.tweet.id_str+"-"+fName:"";

           if(vdoFname.trim().length>0)
           {
             FileExists(vdoFname, d.tweet, function(stat, va, file, lastIdx) {
               orient = !navigator.maxTouchPoints ? 'desktop' : !window.screen.orientation.angle ? 'portrait' : 'landscape';
               video_len = va.extended_entities.media.filter(a=>a.type=="video" || a.type=="animated_gif").length;
               photo_len = va.extended_entities.media.filter(a=>a.type=="photo").length;
               if(video_len>0 && photo_len==0)
               {
                 if(va.extended_entities.media.length>1 && d.tweet.extended_entities.media.length==2)
                 iTxt = "<div class=\"double_img-prev"+wVdoflag+"\">"
                 if(va.extended_entities.media.length>1 && d.tweet.extended_entities.media.length==3)
                 iTxt = "<div class=\"triple_img-prev"+wVdoflag+"\">";
                 if(va.extended_entities.media.length>1 && d.tweet.extended_entities.media.length==4)
                 iTxt = "<div class=\"forth_img-prev"+wVdoflag+"\">";
               }
              ...
              if(stat) 
              {
               ...
               if(va.extended_entities.media.length>1)
               {
                  if(window.opt.active_tab=="tweets")
                  {
                      if(photo_len==0 && video_len>0)
                      {
                        if(window.opt.cache.filter(a=>a.media.id==va.id).length==0)
                        window.opt.cache.push({ "media" : { "id" : va.id, "content" : vdo, "counter" : 1}});
                        else
                        {
                            window.opt.cache.filter(a=>a.media.id==va.id)[0].media.content += vdo;
                            window.opt.cache.filter(a=>a.media.id==va.id)[0].media.counter++;
                        }
                        if(window.opt.cache.filter(a=>a.media.id==va.id)[0].media.counter==video_len)
                        {
                          $(".tab-content #nav-tweets #tweets-data").find("#row_"+va.id).find('p').children(':first').html($(".tab-content #nav-tweets #tweets-data").find("#row_"+va.id).find('p').children(':first').html().replace(va.extended_entities.media[lastIdx].url, "<br/><br/>"+iTxt+window.opt.cache.filter(e=>e.media.id==va.id)[0].media.content+"</div>"));
                          window.opt.cache = window.opt.cache.filter(a=>a.media.id!==va.id)
                        }
                     }
                     else if(photo_len>0 && video_len>0)
                     {
                        if(video_len>1)
                        {
                          if(window.opt.cache.filter(e=>e.media.id==va.id)[0].media.counter==1)
                          {
                            window.opt.cache.filter(e=>e.media.id==va.id)[0].media.content += vdo;
                            window.opt.cache.filter(e=>e.media.id==va.id)[0].media.counter++;
                          }
                          else
                          {
                            content = window.opt.cache.filter(e=>e.media.id==va.id)[0].media.content+vdo;
                            media = window.opt.cache.filter(a=>a.media.id==va.id)[0].media.template;
                           media = media.replace("<!--template-->", content);
                           $(".tab-content #nav-tweets #tweets-data").find("#row_"+va.id).find('p').children(':first').html($(".tab-content #nav-tweets #tweets-data").find("#row_"+va.id).find('p').children(':first').html().replace(va.extended_entities.media[lastIdx].url, media));
                           window.opt.cache = window.opt.cache.filter(a=>a.media.id!==va.id);
                         }
                        }
                        else
                        {
                          template = window.opt.cache.filter(a=>a.media.id==va.id)[0].media.template;
                          media = window.opt.cache.filter(e=>e.media.id==va.id)[0].media.content+vdo;
                          media = template.replace("<!--template-->", media);
                          $(".tab-content #nav-tweets #tweets-data").find("#row_"+va.id).find('p').children(':first').html($(".tab-content #nav-tweets #tweets-data").find("#row_"+va.id).find('p').children(':first').html().replace(va.extended_entities.media[lastIdx].url, media));
                  ...
                  {
                    if(photo_len==0 && video_len>=1)
                    {
                        if(video_len>0)
                        {
                          if(window.opt.cache.filter(a=>a.media.id==va.id).length==0)
                          window.opt.cache.push({ "media" : { "id" : va.id, "content" : vdo, "counter" : 0 }});
                         else
                         {
                          window.opt.cache.filter(a=>a.media.id==va.id)[0].media.content += vdo;
                          window.opt.cache.filter(a=>a.media.id==va.id)[0].media.counter++;
                        ...
                        if((window.opt.cache.filter(a=>a.media.id==va.id)[0].media.counter+1)==video_len)
                        {
                          $(".tab-content #nav-replies #replies-data").find("#row_"+va.id).find('p').children(':first').html($(".tab-content #nav-replies #replies-data").find("#row_"+va.id).find('p').children(':first').html().replace(va.extended_entities.media[lastIdx].url, "<br/><br/>"+iTxt+window.opt.cache.filter(e=>e.media.id==va.id)[0].media.content+"</div>"));
                          window.opt.cache = window.opt.cache.filter(a=>a.media.id!==va.id);
                        }
                    }
                    else if(photo_len>0 && video_len>0)
                    {
                      template = window.opt.cache.filter(e=>e.media.id==va.id)[0].media.template;
                       ...
                       if(window.opt.cache.filter(e=>e.media.id==va.id)[0].media.counter==video_len)
                       {
                         $(".tab-content #nav-replies #replies-data").find("#row_"+va.id).find('p').children(':first').html($(".tab-content #nav-replies #replies-data").find("#row_"+va.id).find('p').children(':first').html().replace(va.extended_entities.media[lastIdx].url, template.replace("<!--template-->", media)));
                         
                         window.opt.cache = window.opt.cache.filter(e=>e.media.id!==va.id);
                       }
                    }
                  }
                  setImgPrev();
               ...
               else
               {
                    if(window.opt.active_tab=="tweets")
                    {
                      $(".tab-content #nav-tweets #tweets-data").find("#row_"+va.id).find('p').children(':first').html($(".tab-content #nav-tweets #tweets-data").find("#row_"+va.id).find('p').children(':first').html().replace(va.extended_entities.media[lastIdx].url, "<div class=\"single_img-prev_vdo\">"+vdo+"</div>"));
                    }
                    else if(window.opt.active_tab=="replies")
                     $(".tab-content #nav-replies #replies-data").find("#row_"+va.id).find('p').children(':first').html($(".tab-content #nav-replies #replies-data").find("#row_"+va.id).find('p').children(':first').html().replace(va.extended_entities.media[lastIdx].url, "<div class=\"single_img-prev_vdo\">"+vdo+"</div>"));
        ...
        else if(d.tweet.extended_entities.media[z].type=="photo")
        {
          ...
          if(photo_len>0)
          {
            ...
            {
              template = "<div class=\"single_img-prev\"><!--template--></div>";
              content = "<img id=\"img-prev_single\" src=\""+imgFname+"\"/>";
            }
            else if(d.tweet.extended_entities.media.length==2)
            {
              template = "<div class=\"double_img-prev"+((typeof(vdo_exist)!="undefined")?vdo_exist:"")+"\"><!--template--></div>"
              if(z==0)
                content = "<img class=\"img-prev_double-1st"+(typeof(vdo_exist)!='undefined'?vdo_exist:"")+"\" src=\""+imgFname+"\"/>";
              if(z==1)
                content = "<img class=\"img-prev_double-2nd"+(typeof(vdo_exist)!='undefined'?vdo_exist:"")+"\" src=\""+imgFname+"\"/>";
            }
            else if(d.tweet.extended_entities.media.length==3)
            {
              template = "<div class=\"triple_img-prev"+((typeof(vdo_exist)!="undefined")?vdo_exist:"")+"\"><!--template--></div>"
              if(z==0)
                content ="<img class=\"img-prev_triple-1st"+(typeof(vdo_exist)!='undefined'?vdo_exist:"")+"\" src=\""+imgFname+"\"/>";
              if(z==1)
                content ="<img class=\"img-prev_triple-2nd"+(typeof(vdo_exist)!='undefined'?vdo_exist:"")+"\" src=\""+imgFname+"\"/>";
              if(z==2)
                content ="<img class=\"img-prev_triple-3rd"+(typeof(vdo_exist)!='undefined'?vdo_exist:"")+"\" src=\""+imgFname+"\"/>";
            }
            else if(d.tweet.extended_entities.media.length==4)
            {
              template = "<div class=\"forth_img-prev"+((typeof(vdo_exist)!="undefined")?vdo_exist:"")+"\"><!--template--></div>";
              if(z==0)
                content = "<img class=\"img-prev_forth-1st"+(typeof(vdo_exist)!='undefined'?vdo_exist:"")+"\" src=\""+imgFname+"\"/>";
              if(z==1)
                content = "<img class=\"img-prev_forth-2nd"+(typeof(vdo_exist)!='undefined'?vdo_exist:"")+"\" src=\""+imgFname+"\"/>";
              if(z==2)
                content = "<img class=\"img-prev_forth-3rd"+(typeof(vdo_exist)!='undefined'?vdo_exist:"")+"\" src=\""+imgFname+"\"/>";
              if(z==3)
                content = "<img class=\"img-prev_forth-4th"+(typeof(vdo_exist)!='undefined'?vdo_exist:"")+"\" src=\""+imgFname+"\"/>";
            }
          }

          if(video_len>0)
          {
            if(window.opt.cache.filter(a=>a.media.id==d.tweet.id).length==0)
            {
              window.opt.cache.push({
                "media" : {
                  "id" : d.tweet.id,
                  "content" : content+desc.join("")+((d.tweet.extended_entities.media.length>2)?"<div class=\"clearfix\"></div>":""),
                  "template" : template,
                  "counter" : (video_len>=3)?0:1
                }
              });
            }
            else
            {
              window.opt.cache.filter(a=>a.media.id==d.tweet.id)[0].media.content += content+desc.join("")+((d.tweet.extended_entities.media.length>2)?"<div class=\"clearfix\"></div>":"");
            }
          }
          else iTxt += content;
        }
    ...
    if(typeof(iTxt)!='undefined' && iTxt.length>0 && video_len==0)
    {
        txt = txt.replace(d.tweet.entities.media[0].url, template.replace("<!--template-->", iTxt+desc.join("")+((d.tweet.extended_entities.media.length>2)?"<div class=\"clearfix\"></div></div>":"</div>"))).replace(/(\r\n|\r|\n)/g, "<br/>");
    }
    delete iTxt;
    delete desc;
    delete vdo_exist;
  ...
  if(searchKey.length>0)
  {
    ...
    if(txt.length>0)
    {
      txt = txt.replace(new RegExp(/(http[s]\:\/\/[a-zA-Z0-9\/\_\-\.\#\&\?\=\+\%]+)|(\@[a-zA-Z0-9\_\-\.]+)|(\#[a-zA-Z0-9\_\-]+)|([a-zA-Z0-9_'\-&]+)/ig), function(word, tag)
      {
        ...
        {
         return word.replace(word, "<a target=\"_blank\" class=\"default-link\" href=\"https://twitter.com/"+word.substring(1,word.length)+"\">"+((subj!=null)?subj:word)+"</a>");
        }
        else if(word.substring(0,4)=="http")
        {
         return word.replace(word, "<a target=\"_blank\" class=\"default-link\" href=\""+word+"\">"+((subj!=null)?subj:word).substring(0,40)+"...</a>");
        }
        else
        {
          ...
          else
          {
            ...
            window.YTD.tweets.part0.filter(e => e.tweet.full_text.substring(0,1)!="@").every((e, index) =>
            {
              if(e.tweet.full_text.substring(0,4)=="RT @" && window.YTD.retweets.part0.length>0 && window.YTD.retweets.part0.length==window.YTD.tweets.part0.filter(a => a.tweet.full_text.substring(0,4)=="RT @").length)
                 e = (window.YTD.retweets.part0.filter(k => k.tweet.id == e.tweet.id && typeof(k.tweet.extended_entities)=='object').length==1)?window.YTD.retweets.part0.filter(k => k.tweet.id == e.tweet.id && typeof(k.tweet.extended_entities)=='object')[0]:(window.YTD.retweets.part0.filter(k => k.tweet.id == e.tweet.id)[0]);
              if(e!=null && count>=bottom && count<up)
              {
                src = updateLinks(e);
                src = updateHashtagUser(src, e);
...
function readData(page)
{
  if(window.YTD.tweets.part0.length>0)
  {
    ...
    if((window.opt.stats.search.tweets.active && window.opt.stats.search.tweets.keyword!=null) || (window.opt.stats.search.replies.active && window.opt.stats.search.replies.keyword!=null))
    {
       ...
       {
           if(window.opt.stats.search.tweets.keyword.toLowerCase().match(new RegExp(/^\d{19}$/)))
             results = window.YTD.tweets.part0.filter(e => e.tweet.id==window.opt.stats.search.tweets.keyword.toLowerCase().match(new RegExp(/^\d{19}$/)) && e.tweet.full_text.substring(0,1)!=="@");
           else
       ...
       {
         if(window.opt.stats.search.replies.keyword.toLowerCase().match(new RegExp(/^\d{19}$/)))
             results = window.YTD.tweets.part0.filter(e => e.tweet.id==window.opt.stats.search.replies.keyword.toLowerCase().match(new RegExp(/^\d{19}$/)) && e.tweet.full_text.substring(0,1)=="@");
           else
...
function preparePosts(page=1, refresh=false)
{
  ...
  if(typeof(window.__THAR_CONFIG.dataTypes.tweets.files[0].fileName)!=='undefined' && (window.opt.active_tab=="tweets" || window.opt.active_tab=="replies"))
  {
    ...
    if(window.YTD.tweets.part0.length==0)
    {
      ...
      loadScriptMarkup(tweetJS, function(stat) 
      {
  ...
  if(typeof(window.__THAR_CONFIG.dataTypes.like.files[0].fileName)!=='undefined')
  {
    ...
    {
      unloadFile(window.__THAR_CONFIG.dataTypes.like.files[0].fileName);
       loadScriptMarkup(window.__THAR_CONFIG.dataTypes.like.files[0].fileName, function(stat)
       {
...
$(document).ready(function ()
{
    ...
    if(typeof(window.__THAR_CONFIG)!=='undefined') {
      ...
      loadScriptMarkup( window.__THAR_CONFIG.dataTypes.profile.files[0].fileName, function(){
          if(window.YTD.profile.part0.length>0  && typeof(window.YTD.profile.part0[0].profile.description.bio)!=='undefined')
          {
            ...
            {
              loadScriptMarkup(window.__THAR_CONFIG.dataTypes.following.files[0].fileName, function() {
            ...
            {
              loadScriptMarkup(window.__THAR_CONFIG.dataTypes.follower.files[0].fileName, function()
              {
      ...
      loadScriptMarkup(window.__THAR_CONFIG.dataTypes.account.files[0].fileName, function(e)
      {
